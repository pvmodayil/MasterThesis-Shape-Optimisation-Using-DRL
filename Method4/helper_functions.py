...
# function to plot the results of g_pts variation
###################################################################################################################################################
def plot_gpts_variation(imgdir:str, initial_gpts:dict, calc_values:dict, nom_values:dict, case:str, time:float) -> None:
    """
    ...
    plt.plot(initial_gpts['g_ptsx']*1000, initial_gpts['g_ptsy'], color = 'red',label='Initial')
...
# function to plot the results of g_pts variation at intermediate stage
###################################################################################################################################################
def plot_gpts_variation_intermediate_stage(imgdir:str, initial_gpts:dict, calc_values:dict, nom_values:dict, case:str, timesteps:float) -> None:
    """
        Function plot the prediction and nominal curves
        Input
        -----------
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"g points {case}: Nominal Energy:{nom_values['energy']}, Prediction Energy:{calc_values['energy']} VAs, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_GPtsPredictionResult_{timesteps}.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Potential Profile {case}: Nominal Energy:{nom_values['energy']}, Prediction Energy:{calc_values['energy']} VAs, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_PotentialPredictionResult_{timesteps}.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Charge Density {case}: Nominal Energy:{nom_values['energy']}, Prediction Energy:{calc_values['energy']}, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_SigmaPredictionResult_{timesteps}.png")
    plt.close()
...
def plot_cubic_bezier(imgdir:str, calc_values:dict, nom_values:dict, control_points:np.ndarray, case:str, time:float) -> None:
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"g points {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']} VAs, Learning Time:{time}")
    plt.savefig(f"{imgdir}/{case}_GPtsPredictionResult.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Potential Profile {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']} VAs, Learning Time:{time}")
    plt.savefig(f"{imgdir}/{case}_PotentialPredictionResult.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Charge Density {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']}, Learning Time:{time}")
    plt.savefig(f"{imgdir}/{case}_SigmaPredictionResult.png")
    plt.close()

# function to plot the results of bezier curve
###################################################################################################################################################
def plot_cubic_bezier_intermediate_stage(imgdir:str, calc_values:dict, nom_values:dict, control_points:np.ndarray, case:str, timesteps:float) -> None:
    """
        Function plot the prediction and nominal curves
        Input
        -----------
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"g points {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']} VAs, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_GPtsPredictionResult_{timesteps}.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Potential Profile {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']} VAs, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_PotentialPredictionResult_{timesteps}.png")
    plt.close()
    ...
    plt.xlabel('x axis [mm]')
    plt.grid(True)
    plt.title(f"Charge Density {case}: Nominal Energy:{nom_values['energy']} VAs, Prediction Energy:{calc_values['energy']}, Timestep:{timesteps}")
    plt.savefig(f"{imgdir}/{case}_SigmaPredictionResult_{timesteps}.png")
    plt.close()
...
def do_prediction(model:Any, env:Any, x_true:np.ndarray, y_true:np.ndarray, imgdir:str, case:str, time_train:float) -> Tuple[dict,dict,np.ndarray]:
    ...
    # number of fourier coefficeints
    ######################################################
    env.num_fs = 500
...
# predict and calculate
##################################################################################################################################################################
def do_prediction_intermediate_stage(model:Any, env:Any, x_true:np.ndarray, y_true:np.ndarray, imgdir:str, case:str, timesteps:float) -> np.ndarray:
    """
        Function to predict and calculate the characteristics of the curve
        Input
        -----------
    ...
    # send for plotting(make sure the x values and y values are in metre scale and not in mm)
    ########################################################
    plot_cubic_bezier_intermediate_stage(imgdir,calc_values,nom_values,control_points,case,timesteps)

    return action

# train
#########################################################################################################################################################################
def train_model(algo:str, model:Any, env:Any, case:str, TIMESTEPS:float, models_dir:str) -> Tuple[Any,float]:
    """
