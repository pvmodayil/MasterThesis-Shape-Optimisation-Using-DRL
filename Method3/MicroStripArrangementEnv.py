...
class MicroStripArrangementEnv(Env,msa):
    ...
    def __init__(self,V0,hw_arra,ht_arra,ht_subs,hw_micrstr,ht_micrstr,er_1,er_2,num_fs,num_pts) -> None:
        ...
        self.action_minimum_energy = np.zeros(action_space_size)
        ...
        """
        self.action_space = Box(low=0, high=1,shape=(action_space_size,), dtype=np.float32)
    ...
    def reward_calc(self,g_ptsx:np.ndarray,g_ptsy:np.ndarray,action:np.ndarray) -> float:
        ...
        -----------------------------------------
        reward:
        =>  not less than line_threshold: reward -> mean(line_threshold - g_ptsy)
        =>  less than line_threshold: reward -> reward + 1e3
            =>  monotone decreasing: reward -> reward + 1e3
            =>  convex: reward -> reward + 1/energy
       ...
       """
        ...
        ##################################################################################################################
        reward = 0
        line_threshold = self.line_equ(g_ptsx)
        
        # below threshold 
        ###################################################################################################################
        if np.all(g_ptsy <= line_threshold) and np.all(g_ptsy >= 0) and np.all(g_ptsx >= self.hw_micrstr):
            ...
            if self.monotonically_decreasing(g_ptsy):
                ...
                if self.current_energy < self.minimum_energy:
                    ...
                    # update action taken for minimum energy
                    # self.action_minimum_energy = action
                # else:
                #     convexity_degree = self.degree_of_convex(g_ptsy)
                #     reward += convexity_degree/len(g_ptsy)
        ...
        ####################################################################################################################
            ...
            # mean error from threshold
            reward += np.mean(self.line_threshold - g_ptsy) 
